//! Minimal TorusAsync example demonstrating the high-level async/await API.
//!
//! NOTE: This is a scaffold — the futures don't yet register real wakers with
//! the backend reactor. They work with executors that busy-loop ready tasks
//! but are not correct for production async runtimes (tokio/async-std).
//! See `async_api.rs` module docs for the tracked waker fix.
//!
//! Run with: `cargo run --example torus_async`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tmpfile = std::env::temp_dir().join("torus_async_demo.txt");
    let data = b"Hello from TorusAsync!\n";
    ...
    // Write to file using the async API
    {
        ...
        #[cfg(windows)]
        let fd = std::os::windows::io::AsRawSocket::as_raw_socket(&file) as i32;
